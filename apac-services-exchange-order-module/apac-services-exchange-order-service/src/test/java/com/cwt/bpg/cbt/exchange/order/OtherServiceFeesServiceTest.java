...
public class OtherServiceFeesServiceTest {
	...
	public void shouldReturnFeesBreakdown() {
		...
		Mockito.when(miscFeeCalculator.calculateFee(Mockito.anyObject(), Mockito.anyDouble()))
	...
	public void shouldReturnAirFeesBreakdown() {
		...
		Mockito.when(hkCalculator.calculateFee(Mockito.anyObject(), Mockito.anyDouble()))
			.thenReturn(new FeesBreakdown());
		
		assertNotNull(service.calculateBspAirFee(new OtherServiceFeesInput()));
	}
